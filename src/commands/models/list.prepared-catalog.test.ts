...
describe("models list published transport", () => {
  it.each(["gateway", "standalone"] as const)(
    "preserves case-distinct published keys and capabilities through %s",
    async (source) => {
      const models: ModelChoice[] = [
        { ...model, input: ["text"] },
        { ...model, id: "reader", name: "Lowercase reader", input: ["text", "image"] },
      ];
      ...
      await list({ agent: "work", all: true, json: true });
      expect(runtime.writeJson).toHaveBeenCalledWith(
        {
          count: 2,
          models: expect.arrayContaining([
            expect.objectContaining({ key: "catalog-provider/Reader", input: "text" }),
            expect.objectContaining({ key: "catalog-provider/reader", input: "text+image" }),
          ]),
        },
        2,
      );
  ...
  it.each(["zai", "z.ai", "z-ai"])(
    "preserves exact %s provider keys and their auth in published default rows",
    async (sourceProvider) => {
      ...
      const configuredModels = ["glm-4.7", "glm-4.8"].map((id) => ({
      ...
      const config: OpenClawConfig = {
        ...
        agents: {
          ...cfg.agents,
          defaults: { model: { primary: `${sourceProvider}/glm-4.7` } },
        },
        models: {
          mode: "replace",
          providers: {
      ...
      owner = {
        ...
        authModes: { [sourceProvider]: "api_key" },
        metadataSnapshot,
      ...
      setPreparedModelRuntimeAuthStore(owner, {
        ...
        profiles: {
          [`${sourceProvider}:test`]: {
            ...
            key: "synthetic-zai-key",
          },
        },
      });
      vi.mocked(runtimeConfig.getRuntimeConfig).mockReturnValue(config);
      ...
      expect(runtime.writeJson).toHaveBeenCalledWith(
        {
          count: 2,
          models: expect.arrayContaining([
            expect.objectContaining({ key: `${sourceProvider}/glm-4.7`, available: true }),
            expect.objectContaining({ key: `${sourceProvider}/glm-4.8`, available: true }),
          ]),
        },
        2,
      );
