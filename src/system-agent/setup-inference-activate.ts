  ...
  return await withSetupInferencePlan(
    {
      ...
      isRemoteProviderAuth:
        params.kind === "provider-auth"
          ? (params.isRemoteProviderAuth ?? params.surface === "gateway")
          : undefined,
      ...(codexCliApiKey ? { codexCliApiKey } : {}),
      deps,
      routeAgentId,
    },
    async (builtPlan, scope): Promise<ActivateSetupInferenceResult> => {
      ...
      const withProbePluginGeneration = <T>(run: () => T): T =>
        probePluginGeneration
          ? withPluginRuntimeGenerationScope(probePluginGeneration, run)
          : run();
      try {
        let plan = builtPlan;
        ...
        if (plan.persistModelRef) {
          const agentRuntimeId =
            plan.selectedAgentRuntimeId ?? resolveSetupAgentRuntimeId(params.kind);
            ...
            async (): Promise<ActivateSetupInferenceResult | undefined> => {
              ...
              try {
                ...
                return {
                  ok: false,
                  status: "unavailable",
                  error: registryRefreshWarning
                    ? `${registryRefreshWarning} ${loadError}`
                    : loadError,
                };
              }
              return undefined;
        ...
        const completeActivation = async (
          ...
          const { metadataSnapshot, modelCatalog } = selection.preparedModelRuntime;
          const routeDeps = {
          ...
          const baselineRoute = await projectInferenceRoute(
            cfg,
            requestedAgentId,
            routeDeps,
            sourceCfg,
          ...
          return {
            ...
            lines,
            ...(params.surface === "gateway" && persistenceState.gatewayRestartRequired
              ? { gatewayRestartRequired: true as const }
              : {}),
          };
        };
        if (scope.selection?.preparedModelRuntime.config === testPlan.config) {
          return await completeActivation(scope.selection);
        }
        // Installation and staged preferences can replace the preparation generation.
        // Acquire its catalog only after those effects; never carry pre-install facts forward.
        return await withProbePluginGeneration(() =>
          withPreparedSimpleCompletionSelection(
            ...
            async (_selection, selection) => await completeActivation(selection),
          ),
        );
      } finally {
        verificationProgress?.stop();
        let codexCleanupError: SetupInferenceActivationIndeterminateError | undefined;
        if (pendingCodexInstall && persistenceState.codexInstallOwnership !== "owned") {
