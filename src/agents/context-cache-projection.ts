...
export function applyConfiguredContextWindows(params: {
  ...
  for (const model of configuredContextModels(params.modelsConfig)) {
    applyConfiguredContextWindow({ ...params, model });
  }
}

function* configuredContextModels(modelsConfig: ModelsConfig | undefined) {
  for (const [providerId, provider] of Object.entries(modelsConfig?.providers ?? {})) {
    ...
    for (const [id, model] of models) {
      yield { ...model, id, provider: normalizeProviderId(providerId) };
    }
  }
}
...
function applyConfiguredContextWindow(params: {
  cache: Map<string, number>;
  windowCache: Map<string, number>;
  model: ContextWindowModelEntry & { provider: string };
}): void {
  ...
  configuredValue.cache.set(
    providerContextTokenCacheKey(params.model.provider, modelId),
...
export async function prepareContextWindowCaches(params: {
  ...
  for (const model of configuredContextModels(params.config.models)) {
