...
function runSdkTool(
  ...
  if (!(result instanceof Promise)) {
    throw new Error(`SDK tool '${tool.name}' did not return a Promise`);
  }
  return result;
}
  ...
  it("keeps Code Mode on a constructor-less transcript-capable host", async () => {
    ...
    const result = await createCopilotToolBridge({
      attemptParams: {
        config: { tools: { codeMode: true } },
        hostCapabilities,
        runId: "run-code-mode-constructorless-host",
        sessionKey: "agent:agent-1:main",
      },
      createOpenClawCodingTools,
    });
...
describe("createCopilotToolBridge tool conversion", () => {
  it("keeps SDK 1.0.11 dispatch pending on the exact durability receipt", async () => {
    ...
    const onSuspendableToolCompleted: NonNullable<
      CopilotToolBridgeInput["onSuspendableToolCompleted"]
    > = vi.fn(({ result, toolCallId }) => {
      expect(asOptionalRecord(asOptionalRecord(result)?.details)?.status).toBe("waiting");
      return toolCallId === "exec-waiting-1" ? durability.promise : Promise.resolve();
    });
    const sourceTool = makeTool(
